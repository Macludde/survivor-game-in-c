...
void TurnAwayEnemiesFromEachother(Enemy *enemy, Enemy *collided) {
  ...
  float overlap =
      (totalSize - Vector2Length(delta)) / totalSize;  // lower => more overlap

  float rotationSpeed = 4 * ENEMY_ROTATION_SPEED * GetFrameTime() * overlap;
  float ratio = (collided->size / (collided->size + enemy->size));
  enemy->targetVelocity = Vector2Lerp(
      enemy->targetVelocity, Vector2Normalize(delta), rotationSpeed * ratio);
  collided->targetVelocity =
      Vector2Lerp(collided->targetVelocity, Vector2Normalize(invDelta),
                  rotationSpeed * (1 - ratio));
}

#define ENEMY_INTERNAL_KNOCKBACK 3.0f  // higher => bigger jumps, more jitter?
// ratio between 0 and 1. 1 Means enemy is the only one moving, 0 means entity
// is the only one moving
void EnemyCollisionWithEntity(Enemy *enemy, Vector2 *entityPos,
                              float entitySize, float ratio, Vector2 delta,
                              float distanceSqr, float desiredDistance) {
  float distance = sqrtf(distanceSqr);
  float scale =
      (desiredDistance - distance) * ENEMY_INTERNAL_KNOCKBACK * GetFrameTime();
  Vector2 totalForce = Vector2Scale(Vector2Normalize(delta), scale);
  enemy->pos = Vector2Add(enemy->pos, Vector2Scale(totalForce, ratio));
  *entityPos = Vector2Add(
      *entityPos,
      Vector2Scale(totalForce, ratio - 1));  // -1 * (1-ratio) = ratio - 1
}
void EnemyEnemyCollision(Enemy *enemy1, Enemy *enemy2, Vector2 delta,
                         float distanceSqr, float desiredDistance) {
  float ratio = (enemy2->size / (enemy2->size + enemy1->size));
  EnemyCollisionWithEntity(enemy1, &enemy2->pos, enemy2->size, ratio, delta,
                           distanceSqr, desiredDistance);
  TurnAwayEnemiesFromEachother(enemy1, enemy2);
}

void EnemyTreeCollision(Enemy *enemy, Vector2 tree, Vector2 delta,
                        float distanceSqr, float desiredDistance) {
  float distance = sqrtf(distanceSqr);
  ...
  enemy->targetVelocity = Vector2Lerp(
      enemy->targetVelocity, Vector2Reflect(enemy->targetVelocity, direction),
      3 * GetFrameTime());
}
...
void CollisionsWithOtherEnemies(Enemy *enemy, Enemy *allEnemies, int startAt,
  ...
  for (int j = startAt; j < highestEnemyIndex; ++j) {
    ...
    if (distanceSqr < desiredDistance * desiredDistance)
      EnemyEnemyCollision(enemy, &allEnemies[j], delta, distanceSqr,
                          desiredDistance);
  }
}
...
void CollisionWithPlayer(Enemy *enemy, Player *player) {
  ...
  if (distanceSqr < desiredDistance * desiredDistance) {
    ...
    // EnemyCollisionWithEntity(enemy, &player->pos, player->size, ratio, delta,
    // distanceSqr, desiredDistance);

    // slow down enemy, knockback player.
    // m1*v1 = m2*v2
    float scale = ENEMY_PLAYER_KNOCKBACK;
    // player->velocity = Vector2Lerp(player->velocity, enemy->velocity,
    // ENEMY_PLAYER_KNOCKBACK); enemy->velocity = Vector2Lerp(enemy->velocity,
    // playerVelocity, ENEMY_PLAYER_KNOCKBACK); v2 = v1*m1/m2
    player->velocity = Vector2Add(
        player->velocity,
        Vector2Scale(enemy->velocity,
                     ENEMY_PLAYER_KNOCKBACK * (enemy->size / player->size)));
